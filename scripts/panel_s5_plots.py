...
def stacked_run_plots():
    for l in [4,6]:
        for n in [6]:
            success_folder = output_folder+f"l_{l}_n_{n}_success/"
            failure_folder = output_folder+f"l_{l}_n_{n}_failure/"
            os.makedirs(success_folder, exist_ok=True)
            os.makedirs(failure_folder, exist_ok=True)
            for i in range(100):
                dir = f"data/kv_10_l_{l}_n_{n}/{i:03d}/"
                # print("Checking: ", dir)
                if not os.path.isfile(dir+"costs.txt"):
                    continue
                if not os.path.isfile(dir+"q_values.txt"):
                    continue
                if not os.path.isfile(dir+"SD_s0.csv"):
                    continue
                ...
                label_to_data[r"$SD(s_0)$"] = {"x": df["iter"]+np.ones_like(df["iter"]), "y": df["SD_s0"], "loc": "bottom right"}
