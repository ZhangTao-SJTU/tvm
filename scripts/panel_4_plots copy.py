...
def single_run_stacked_plot(dirlist,
                             output_folder,
                             xlim = [1,2500],
                            ylim_top = [1e-8,1e1],
                            ylim_bottom = [0.5,1.1],
                            yticks_bottom = [0.6,0.8,1],
                            title = None,
                            xLog = True,
                            yLog_top = True,
                            yLog_bottom = False,
                            xlabel = "Iterations",
                            ylabel_top = r"$<|1-\sigma_{T}/\sigma|>$",
                            ylabel_bottom = r"$Q_2$"):
    for dir in dirlist:
        ...
        plotter = stacked_plot(
                xlim = xlim,
                ylim_top = ylim_top,
                ylim_bottom = ylim_bottom,
                yticks_bottom = yticks_bottom,
                xLog = xLog,
                yLog_top = yLog_top,
                yLog_bottom = yLog_bottom,
                xlabel = xlabel,
                ylabel_top = ylabel_top,
                ylabel_bottom = ylabel_bottom,
                title = title
        )
...
def main():
    ...
    for l in [4,5,6]:
        ...
        for n in [1,2,3,4,5,6]:
            ...
            os.makedirs(output_folder,exist_ok=True)
            dirlist = find_complete_runs(["data/kv_10_l_{}_n_{}/{:03d}/".format(l,n,i) for i in range(100)])
            single_run_stacked_plot(dirlist, output_folder, title=title)
