...
def s0_histogram_periodic_combined():
    ...
    for l in [4]:
        for n in [2,3,4]:
            savefile = "Panel_3/s0_histogram_periodic_l_{}_n_{}.png".format(l,n)
            label_to_data = {}
...
def error_to_iters_single_pattern():
    for l in [4]:
        ...
        for n in [1,2,4]:
            savefile = "Panel_3/error_to_iters_periodic_l_{}_n_{}.png".format(l,n)
            # title = r"$n_{total} = $"+"{}".format(l**3)
...
def final_iteration_to_final_overlap_periodic():
    ...
    for n in [2,3,4]: 
        label_to_dir = {}
        savefile = "Panel_3/final_iterations_to_final_overlap_l_{}_n_{}.png".format(l,n)
        # title = r"$n_{total} = $"+"{}".format(l**3)
        dirlist = find_complete_runs(["data/kv_10_l_{}_n_{}/{:03d}/".format(l,n,i) for i in range(100)])
...
def SD_s0_scatter_all_patterns():
    l = 4
    n_cells =[2,3,4]
    header = "data/kv_10_l_{}_p".format(l)
    savefile = "Panel_3/SD_s0_scatter_patterns_l_{}.png".format(l)
    label_to_dir = {}
    for n in n_cells:
        #single pattern
