...
def create_sub_file(script,dir):
    ...
    lines.append("request_memory = 250 MB\n")
    lines.append("queue \n")
...
def copy_init_folder(destination_dir):
    os.system("cp -r init/kv_10_l_4 {}".format(destination_dir))
    print("created {}".format(destination_dir))
    
def main():
    script = "multiple_patterns_new.py"
    tolerance = 1e-6
    n_runs = 100
    max_iters = 50000
    clear_interval = 100
    ...
    subpatterns = [[1,1],[1,2],[1,3],[2,2],[1,1,1],[1,1,1,1],[1,1,2]]
    for subpattern_n_cells in subpatterns:
        n_cells = sum(subpattern_n_cells)
        experiment = "/home/mameen/new_kv_10_l_4_p"
        for p in subpattern_n_cells:
            experiment += "_{}".format(p)
        experiment+="/"
        copy_init_folder(experiment)
        for i in range(n_runs):
            ...
            np.savetxt("{}subpattern_n_cells".format(run_dir),subpattern_n_cells,fmt="%d")
...
def check_costs():
    subpatterns = [[1,1],[1,2],[1,3],[2,2],[1,1,1],[1,1,1,1],[1,1,2]]
    for subpattern_n_cells in subpatterns:
        experiment = "/home/mameen/kv_10_l_4_p"
        for p in subpattern_n_cells:
            experiment += "_{}".format(p)
        experiment+="/"
        for i in range(100):
            ...
            if os.path.isfile(run_dir + "output.txt"):
