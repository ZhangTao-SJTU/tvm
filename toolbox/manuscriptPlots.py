...
class plot:
    ...
    def histogram_from_dataframe(self, df, bins = 50, color = "#7d878a", label = "plot"):
        df.plot(
            ax = self.ax,
            kind = "hist",
            density = True,
            ...
            color = color,
            alpha = 0.5)
        df.plot(ax = self.ax, kind = "kde",color = color, label = "_hidden")

    def histogram_from_array(self, array, bins = 50, color = "#7d878a", label = "plot"):
        df = pd.DataFrame(array)
        self.histogram_from_dataframe(df, bins = bins, color = color, label = label)
