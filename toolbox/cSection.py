from toolbox import topology,functions
import numpy as np
import pyvtk
import os
...
def makeSampleCrossSection(sample, center:np.ndarray = np.array([0,0,0]), normal:np.ndarray = np.array([1,0,0]),out_folder = "cSections/"):
    ...
    #####################################################################
    output_dir = sample.config_dir_+ out_folder
    if not os.path.isdir(output_dir):
        os.makedirs(output_dir)
    #####################################################################
    # Step 1: identify the cells that are intersected by the cross section plane. 
    ...
    # of the vectors from the center to the vertices of the cell
    ...
    for cellID,cell in sample.cells_.items():
        if sample.tissueType_ == "spheroid": 
            if not cell.type_:
                continue
            # if cell.is_surface_:
            #     continue
        ...
        for vertexID in cell.vertices_:
            vector_from_center = np.subtract(sample.vertices_[vertexID].position_,center)                                 
            sign = np.sign(np.dot(normal,vector_from_center))
            signs.append(sign)
            if int(sign) == 0:
                raise ValueError("Error: a vertex of cell {} on cross section".format(cellID))
        if len(set(signs)) == 1:
            continue
        cell.is_mother_ = True
    ...
    for polygonID,polygon in new_polygons.items():
        ...
        for i,vertexID in enumerate(polygon.vertices_):
            ...
            for vertexID in new_triangular_polygon.vertices_:
                vector_from_center = np.subtract(new_vertices[vertexID].position_, center)
                sign = np.sign(np.dot(normal,vector_from_center))
                if int(sign) == 0:
                    raise ValueError("Error: the polygon center for polygon {} on cross section".format(polygonID))
                signs.append(sign)
            if len(set(signs)) == 1:
                continue
    ...
    for edgeID, edge in new_edges.items():
        ...
        t = np.dot(normal,np.subtract(center,r_0)) / np.dot(normal,np.subtract(r_1,r_0))
        if t <=1 and t>=0:
    ...
    for cellID, cell in sample.cells_.items():
        ...
        new_cell.vtk_scalar_ = cell.vtk_scalar_
        new_cell.polygons_ = []
        for polygonID in cell.polygons_:
    ...
    for cellID, cell in new_cells.items():
        ...
        intersection_polygons[id].vtk_scalar_ = cell.vtk_scalar_
        for polygonID in cell.polygons_:
    ...
    for polygonID,polygon in intersection_polygons.items():
        ...
        polygon.vertices_ = functions.resolve_polygon_edge_connectivity(vertices)
    ...
    for polygonID,polygon in intersection_polygons.items():
        Polygons_.append([vertexID for vertexID in polygon.vertices_])
        cellscalars.append(polygon.vtk_scalar_)
