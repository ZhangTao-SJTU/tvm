...
def bounding_ellipsoid(P, tolerance):
...
def run(runid, remote_disk, timesteps):
    if remote_disk == 1:
        ...
        runDir = "/Volumes/Seagate/SJTU/VertexModel/run/{:06d}".format(runid)
    else:
    ...
    with open(readFilePath, "r") as file:
        ...
        for line in file:
            ...
            if lineSplit[0] == "time":
                ...
                polygons = {}
                continue
            if timestamp not in cells_frames:
                continue
            ...
            if verticesFlag:
                if lineSplit[-1] == "nan" or lineSplit[-1] == "-nan":
                    if remote_disk == 1:
                        ...
                        dumpDir = "/Volumes/Seagate/SJTU/VertexModel/run/{:06d}".format(runid)
                    else:
                    ...
                    for polygonID in polygons:
                        ...
                        if polygon.layer_ == 1:
                            for cell in polygon.cells_:
                                if cell.layer_ == 0:
                                    cell.layer_ = 2
                    # update cell.neighbors_
                    for polygonID in polygons:
                        polygon = polygons[polygonID]
                        if len(polygon.cells_) == 2:
                            cell0 = polygon.cells_[0]
                            cell1 = polygon.cells_[1]
                            if cell1.id_ not in cell0.neighbors_:
                                cell0.neighbors_.append(cell1.id_)
                            if cell0.id_ not in cell1.neighbors_:
                                cell1.neighbors_.append(cell0.id_)
    ...
    for timestamp in cells_frames:
        ...
        for cellID in cells_frames[timestamp]:
            ...
            cellSurfaces.append(cell.layer_)
            neighbors.append(list(cell.neighbors_))

        dumpCellOrientation(runid, timestamp, cellIDs, centers, orientations, cellSurfaces, neighbors, remote_disk)
...
def dumpCellOrientation(runid, timestamp, cellIDs, centers, orientations, cellSurfaces, neighbors, remote_disk):
    if remote_disk == 1:
        dumpDir = "/mnt/hgfs/E/SJTU/VertexModel/run/{:06d}".format(runid)
    elif remote_disk == 2:
        dumpDir = "/Volumes/Seagate/SJTU/VertexModel/run/{:06d}".format(runid)
