...
class Spheroid(Sample):
    ...
    @classmethod
    def from_dir_and_time(cls, config_dir, simulation_time):
        sample = cls()
        ...
        sample.calculate_spheroid_attributes()
        return sample
    ...
    def from_config(cls, config_dir, input_filename = "sample.topo"):
        ...
        # writing an analog for PeriodicTissue.load_periodic_tissue_from_file()
        sample.load_spheroid_from_file()
        return sample
    
    def load_spheroid_from_file(self):
        self.load_config()
        self.load_conf_file()
        ...
        self.calculate_spheroid_attributes()
    ...
    def calculate_spheroid_attributes(self):
    ...
    def write_surface_vtk(self,filename = "surface_cells.vtk"):
        surface_cells = [cellID for cellID,cell in self.cells_.items() if cell.is_surface_]
        self.write_cell_collection_vtk(surface_cells, filename)

    def write_spheroid_vtk(self,filename = "spheroid.vtk"):
        spheroid_cells = [cellID for cellID,cell in self.cells_.items() if cell.type_]            
        self.write_cell_collection_vtk(spheroid_cells, filename)
    #legacy code for write_surface_vtk    
    # def write_surface_vtk(self):
