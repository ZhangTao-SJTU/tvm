...
def calculate_tetrahedral_inertia_tensor(vertices):
    ...
    a=determinant*(y0**2+y1**2+y2**2+z0**2+z1**2+z2**2+y0*y1+y0*y2+y1*y2+z0*z1+z0*z2+z1*z2)/60
    b=determinant*(x0**2+x1**2+x2**2+z0**2+z1**2+z2**2+x0*x1+x0*x2+x1*x2+z0*z1+z0*z2+z1*z2)/60
    c=determinant*(x0**2+x1**2+x2**2+y0**2+y1**2+y2**2+x0*x1+x0*x2+x1*x2+y0*y1+y0*y2+y1*y2)/60
    a_prime=determinant*(y0*z0+y0*z1+y0*z2+y1*z0+y1*z1+y1*z2+y2*z0+y2*z1+y2*z2)/120
    b_prime=determinant*(x0*z0+x0*z1+x0*z2+x1*z0+x1*z1+x1*z2+x2*z0+x2*z1+x2*z2)/120
    c_prime=determinant*(x0*y0+x0*y1+x0*y2+x1*y0+x1*y1+x1*y2+x2*y0+x2*y1+x2*y2)/120

    inertia_tensor = np.array([[a,-b_prime,-c_prime],[-b_prime,b,-a_prime],[-c_prime,-a_prime,c]])
...
def calculateElongationAxis(sample:tissueSample.Sample, cellID:int):
    ...
    for polygonID in cell.polygons_:
        ...
        for edgeID in sample.polygons_[polygonID].edges_:
            ...
            inertia_tensor+=calculate_tetrahedral_inertia_tensor(tetrahedral_vertices)
