...
def rearrange_polygon_vertices_for_cell(sample, cellID:int) -> None:
...
def calculate_volume_term(sample, cellID:int):
    cell = sample.cells_[cellID]
...
def calculate_surface_area_term(sample, cellID:int):
    cell = sample.cells_[cellID]
...
def calculate_gamma_term(sample, cellID:int):
    cell = sample.cells_[cellID]
...
def calculate_stress_tensor(sample, cellID:int, COM_center = False):
    if COM_center:
        calculate_stress_tensor_COM_center(sample,cellID)
    else:
        ...
        stressTensor = calculate_stress_tensor_triangulation(sample, cellID)
        return stressTensor
def calculate_max_shear_stress(sample, cellID:int,COM_center = False):
    stress = calculate_stress_tensor(sample,cellID,COM_center)
...
def calculate_stress_tensor_COM_center(sample, cellID:int):
    rearrange_polygon_vertices_for_cell(sample, cellID)
...
def calculate_stress_tensor_triangulation(sample, cellID:int):
    cell = sample.cells_[cellID]
    ...
    for polygonID in cell.polygons_:
        ...
        for edgeID in polygon.edges_:
            ...
            ## No need to resolve the issue of orientation here; as the tensor term is quadratic in a_poly
            tensor_term = np.add(
                tensor_term,
