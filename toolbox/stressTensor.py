...
def rearrange_polygon_vertices_for_cell(
    ...
    # print("Checking orientation of {} polygons; ids {}".format(len(cell.polygons_),cell.polygons_) )
    for polygonID in cell.polygons_:
        ...
        for i in range(len(polygon.vertices_)):
            ...
            vector1 = np.subtract(sample.vertices_[v1].position_, cell.center_)
            vector2 = np.subtract(sample.vertices_[v2].position_, cell.center_)
            cross_product = np.cross(vector1, vector2)
            sum_of_cross_products = np.add(sum_of_cross_products, cross_product)
        sign = np.sign(np.dot(sum_of_cross_products,np.subtract(polygon.center_, cell.center_)))
        if sign == -1:
...
def calculate_surface_area_term(sample:tissueSample.Sample, cellID:int):
    ...
    for polygonID in cell.polygons_:
        ...
        for i in range(len(polygon.vertices_)):
            ...
            tensor_product = np.subtract(
                tensor_product,
                np.outer(edgeVector,edgeVector) / area)
    ...
    for polygonID in cell.polygons_:
        ...
        for i in range(len(polygon.vertices_)):
            ...
            sum = np.add(sum, np.cross(edgeVector, areaVector) / np.linalg.norm(areaVector))
