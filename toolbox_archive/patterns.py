...
class Patterns(Training):
    ...
    def set_target_stresses(self,cellToStress):
        self._target_stresses = cellToStress
    ...
    def calculate_max_shear_stresses(self):
        for cellID in self._target_cells:
            ...
            if cell.crossBoundary_:
                continue
            max_shear = stress.calculate_max_shear_stress(self._config, cellID, COM_center=True)
    ...
    def evaluate_cost(self):
        ...
        for cellID in self._target_cells:
            cell = self._config.cells_[cellID]
            self._cost += multiplier * (cell.max_shear_stress_ - self._target_stresses[cellID]) ** 2
    ...
    def clamp_target_cells(self,clamping_tolerance = 1e-3, max_iters = 20):
        ...
        for iter in range(max_iters):
            ...
            for cellID in self._target_cells:
                ...
                if abs(current_stress - self._target_stresses[cellID]) > clamping_tolerance:
            ...
            for cellID in needs_clamping:
                ...
                target_stress = self._target_stresses[cellID] * (1 + correction_factor * (self._target_stresses[cellID] - current_stress))
                ...
                while abs(stress_change - del_stress) > self._tolerance:
                    ...
                    #     print(iter_counter, cell.s0_)
                    stress_tensor = stress.calculate_stress_tensor_COM_center(self._config,cellID)
                    ...
                    if iter_counter < 2:
                        continue
                    if lower_bound<= s0_min:
                        lower_bound = s0_min
                        print("WTF!")
                        break
                    elif upper_bound>=s0_max:
                        upper_bound=s0_max
                        print("WTF!")
                        break
                cell.s0_ = (upper_bound + lower_bound) / 2
    ...
    def run(self):
        ...
        while self._cost > self._tolerance:
            self.single_iteration(clamp_tol = clamp_tol)
            self.evaluate_cost()
            self._cost_values.append(self._cost)
            print("Iteration: {:d}, Cost: {:.2e}".format(self._iter_counter,self._cost))
            self._iter_counter += 1
            if not len(self._cost_values)>1:
                continue
            cost_change = abs((self._cost_values[-1]-self._cost_values[-2])/self._cost_values[-1])
            if cost_change < 1e-7:
                clamp_tol/=10
